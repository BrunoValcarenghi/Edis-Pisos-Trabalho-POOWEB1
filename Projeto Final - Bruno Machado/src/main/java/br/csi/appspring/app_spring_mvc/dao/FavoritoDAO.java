...
public class FavoritoDAO {
    ...
    public ArrayList<Favorito> getFavoritos(int usuario_id) throws SQLException {
        ...
        // Mudamos para usar JOINs explícitos e o caractere '?' no lugar do ID
        String sql = "SELECT f.*, p.marca, p.modelo FROM favorito f " +
                "INNER JOIN usuario u ON u.id = f.usuario_id " +
                "INNER JOIN produto p ON p.id = f.produto_id " +
                "WHERE f.usuario_id = ?";

        try (Connection conn = ConexaoDB.getConexao();
             PreparedStatement stmt = conn.prepareStatement(sql)) {

            // Injeta dinamicamente o ID do usuário que está logado na sessão
            stmt.setInt(1, usuario_id);
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    // Dica: use o alias ou o nome correto da tabela favorito para o ID dela
